...
planoSaude_with_ClinMedExameRouter.get(
  "/ps_clin_med_exam",
  verifyJWT,
  (req, res, next) => {
  ...
  (req, res, next) => {
    async function findOne() {
      PlanoSaude_ClinMedExame.findById(req.params.id)
        .populate("clinicaMedicoExameId")
        .populate("planoDeSaudeId")
        .then((dados) => {
          res.status(200);
          res.json(dados);
        })
...
planoSaude_with_ClinMedExameRouter.post(
  "/ps_clin_med_exam",
  verifyJWT,
  (req, res, next) => {
...
planoSaude_with_ClinMedExameRouter.put(
  "/ps_clin_med_exam/:id",
  verifyJWT,
  (req, res, next) => {
    async function atualizar() {
      try {
        PlanoSaude_ClinMedExame.findByIdAndUpdate(
          req.params.id,
          req.body,
          function (erro) {
            if (erro) {
...
//DELETAR um registro
planoSaude_with_ClinMedExameRouter.delete(
  "/ps_clin_med_exam/:id",
  verifyJWT,
  (req, res, next) => {
    async function deletar() {
      PlanoSaude_ClinMedExame.findByIdAndDelete(req.params.id)
        .then((dados) => {
          if (dados) {

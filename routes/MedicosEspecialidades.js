...
medicoEspecialidadesRouter.get("/medicoEsp", verifyJWT, (req, res, next) => {
...
medicoEspecialidadesRouter.get(
  "/medicoEsp/:id",
  verifyJWT,
  (req, res, next) => {
    async function findOne() {
      MedicosEspecialidades.findById(req.params.id)
        .populate("medicoId")
        .populate("especialidades.especialidadeId")
        .then((dados) => {
          res.status(200);
          res.json(dados);
        })
...
medicoEspecialidadesRouter.post("/medicoEsp", verifyJWT, (req, res, next) => {
...
medicoEspecialidadesRouter.put(
  "/medicoEsp/:id",
  verifyJWT,
  (req, res, next) => {
    async function atualizar() {
      try {
        MedicosEspecialidades.findByIdAndUpdate(
          req.params.id,
          req.body,
          function (erro) {
            if (erro) {
...
//DELETAR um registro
medicoEspecialidadesRouter.delete(
  "/medicoEsp/:id",
  verifyJWT,
  (req, res, next) => {
    async function deletar() {
      MedicosEspecialidades.findByIdAndDelete(req.params.id)
        .then((dados) => {
          if (dados) {

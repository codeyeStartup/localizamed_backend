...
planosDeSaudeRouter.get("/planos_de_saude", verifyJWT, (req, res, next) => {
...
planosDeSaudeRouter.get("/planos_de_saude/:id", verifyJWT, (req, res, next) => {
  async function findplano_de_saude() {
    Planos_de_saude.findById(req.params.id)
      .then((plano_de_saude) => {
        res.status(200);
...
planosDeSaudeRouter.post("/planos_de_saude", verifyJWT, (req, res, next) => {
...
planosDeSaudeRouter.delete(
  "/planos_de_saude/:id",
  verifyJWT,
  (req, res, next) => {
    async function deletarPlanos_de_saude() {
      Planos_de_saude.findByIdAndDelete(req.params.id)
        .then((plano_de_saude) => {
          if (plano_de_saude) {
...
planosDeSaudeRouter.put("/planos_de_saude/:id", verifyJWT, (req, res, next) => {
  async function atualizarPlanos_de_saude() {
    try {
      Planos_de_saude.findByIdAndUpdate(req.params.id, req.body, function (
        erro
      ) {
        if (erro) {
          res.send(erro);

...
exames_consultasRouter.get("/exames_consultas", verifyJWT, (req, res, next) => {
...
exames_consultasRouter.get(
  "/exames_consultas/:id",
  verifyJWT,
  (req, res, next) => {
    async function findExame_consulta() {
      Exames_Consultas.findById(req.params.id)
        .then((exames_consultas) => {
          res.status(200);
        ...
        .catch((erro) => {
          if (erro) {
            res
              .status(417)
              .send({ message: "Nenhum exame/consulta encontrado" });
            throw erro;
          }
        });
    }
...
exames_consultasRouter.delete(
  "/exames_consultas/:id",
  verifyJWT,
  (req, res, next) => {
    async function deletarExames_consultas() {
      Exames_Consultas.findByIdAndDelete(req.params.id)
        .then((exame_consulta) => {
          if (exame_consulta) {
...
exames_consultasRouter.put(
  "/exames_consultas/:id",
  verifyJWT,
  (req, res, next) => {
    async function atualizarExames_Consultas() {
      try {
        Exames_Consultas.findByIdAndUpdate(req.params.id, req.body, function (
          erro
        ) {
          if (erro) {
            res.send(erro);

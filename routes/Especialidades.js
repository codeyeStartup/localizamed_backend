...
especialidadesRouter.get("/especialidades", verifyJWT, (req, res, next) => {
...
especialidadesRouter.get("/especialidade/:id", verifyJWT, (req, res, next) => {
  async function findEspecialidade() {
    Especialidades.findById(req.params.id)
      .then((especialidade) => {
        res.status(200);
...
especialidadesRouter.post("/especialidade", verifyJWT, (req, res, next) => {
...
especialidadesRouter.delete(
  ...
  (req, res, next) => {
    async function deletarEspecialidade() {
      Especialidades.findByIdAndDelete(req.params.id)
        .then((especialidade) => {
          if (especialidade) {
...
especialidadesRouter.put("/especialidade/:id", verifyJWT, (req, res, next) => {
  async function atualizarEspecialidade() {
    try {
      Especialidades.findByIdAndUpdate(req.params.id, req.body, function (
        erro
      ) {
        if (erro) {
          res.send(erro);
